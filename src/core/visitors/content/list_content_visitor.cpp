...
void ListContentVisitor::operator()(const Subclass& subclass) {
    ...
    string_list.push_back(
        fmt::format("{} ({}) [{} SUBCLASS]", subclass.get_name(), subclass.get_source_info().name, cls.get_name())
    );
}
void ListContentVisitor::operator()(const Species& species) {
    string_list.push_back(fmt::format("{} ({}) [SPECIES]", species.get_name(), species.get_source_info().name));
}
...
void ListContentVisitor::operator()(const Item& item) {
    if (item.requires_attunement()) {
        string_list.push_back(
            fmt::format("{} ({}) [ITEM] requires attunement", item.get_name(), item.get_source_info().name)
        );
    } else {
        string_list.push_back(fmt::format("{} ({}) [ITEM] no attunement", item.get_name(), item.get_source_info().name)
        );
    }
}
...
void ListContentVisitor::operator()(const Feature& feature) {
    string_list.push_back(fmt::format(
        "{} ({}) [FEATURE] : {}", feature.get_name(), feature.get_source_info().name, feature.get_source_info().name
    ));
}
...
void ListContentVisitor::operator()(const Choosable& choosable) {
    string_list.push_back(fmt::format(
        ...
        choosable.get_source_info().name
    ));
}
