...
dnd::Character dnd::Character::create(dnd::CharacterData&& data, const ContentHolder& content) {
    ...
    return Character(
        ...
        std::move(base_ability_scores), std::move(basis), std::move(progression)
    );
}
...
dnd::Character::Character(
    ...
    dnd::Progression&& progression
) noexcept
    : FeatureHolder(std::move(name), std::move(description), std::move(source_path), std::move(features)),
      base_ability_scores(std::move(base_ability_scores)), basis(std::move(basis)),
