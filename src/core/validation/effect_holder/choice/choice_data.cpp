...
static dnd::Errors validate_effect_choice(const dnd::ChoiceData& data, const dnd::ValidationData* parent) {
    dnd::Errors errors;
    ...
    for (const auto& explicit_choice : data.explicit_choices) {
        ...
        effect_data.effect_str = explicit_choice;
    ...
    if (!data.group_names.empty()) {
        errors.add_validation_error(
            dnd::ValidationErrorCode::INVALID_ATTRIBUTE_VALUE, parent,
            fmt::format(
                "Choice for '{}' implies the selection of an effect which prohibits group names.", data.attribute_name
            )
        );
    }
    return errors;
...
dnd::Errors dnd::ChoiceData::validate() const {
    ...
    switch (type) {
        ...
        case ChoiceType::EFFECT:
            errors += validate_effect_choice(*this, parent);
            break;
        default:
            break;
    }
