...
#include <core/validation/validation_error.hpp>
    ...
    for (const auto& [action_name, action_description] : actions) {
        if (action_name.empty()) {
            errors.add_validation_error(
                ValidationErrorCode::INVALID_ATTRIBUTE_VALUE, this, "Action name cannot be empty"
            );
        }
        if (action_description.empty()) {
            errors.add_validation_error(
                ValidationErrorCode::INVALID_ATTRIBUTE_VALUE, this, "Action description cannot be empty"
            );
        }
    }
    return errors;
