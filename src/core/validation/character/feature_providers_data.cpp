...
FeatureProvidersData::FeatureProvidersData(std::shared_ptr<const ValidationData> parent) noexcept
...
static Errors validate_feature_providers_raw(const FeatureProvidersData& data) {
    ...
    if (data.species_name.empty()) {
        errors.add_validation_error(
            ValidationError::Code::INVALID_ATTRIBUTE_VALUE, data.get_parent(), "Species name cannot be empty."
        );
    }
    if (data.class_name.empty()) {
        errors.add_validation_error(
            ValidationError::Code::INVALID_ATTRIBUTE_VALUE, data.get_parent(), "Class name cannot be empty."
        );
    }
    return errors;
}
...
static Errors validate_feature_providers_relations(const FeatureProvidersData& data, const Content& content) {
    ...
    if (!species_optional.has_value()) {
        errors.add_validation_error(
            ...
            fmt::format("Species '{}' does not exist.", data.species_name)
        );
        ...
        errors.add_validation_error(
            ValidationError::Code::INVALID_RELATION, data.get_parent(),
            fmt::format("Species '{}' requires subspecies.", data.species_name)
        );
    }
    ...
    if (!class_optional.has_value()) {
        errors.add_validation_error(
            ValidationError::Code::RELATION_NOT_FOUND, data.get_parent(),
            fmt::format("Class '{}' does not exist.", data.class_name)
        );
