...
static Errors validate_subclass_relations_nonrecursively(const Subclass::Data& data, const Content& content) {
    Errors errors;
    if (content.get_subclass_library().contains(data.get_key())) {
        errors.add_validation_error(
    ...
    for (const SubclassFeature::Data& feature_data : data.features_data) {
        if (content.find_class_feature(feature_data.get_key())) {
            errors.add_validation_error(
                ValidationError::Code::INVALID_ATTRIBUTE_VALUE,
    ...
    if (data.class_name != cls.get_name()) {
        errors.add_validation_error(
    ...
    } else if (data.spellcasting_data.is_spellcaster && cls.has_spellcasting()) {
        errors.add_validation_error(
            ValidationError::Code::INVALID_RELATION,
            fmt::format(
