...
void dnd::ListVisitor::visit(const Character* character_ptr) {
    string_list.emplace_back(fmt::format(
        "{} [CHARACTER] : Level {} {} {}", character_ptr->get_name(), character_ptr->get_level(),
        character_ptr->class_ptr->get_name(), character_ptr->race_ptr->get_name()
    ));
}
...
void dnd::ListVisitor::visit(const CharacterSubclass* character_subclass_ptr) {
    string_list.emplace_back(
        fmt::format("{} [{} SUBCLASS]", character_subclass_ptr->get_name(), character_subclass_ptr->class_name)
    );
}
...
void dnd::ListVisitor::visit(const CharacterSubrace* character_subrace_ptr) {
    string_list.emplace_back(
        fmt::format("{} [{} SUBRACE]", character_subrace_ptr->get_name(), character_subrace_ptr->race_name)
    );
}
...
void dnd::ListVisitor::visit(const Spell* spell_ptr) {
    string_list.emplace_back(fmt::format("{} [SPELL] : {}", spell_ptr->get_name(), spell_ptr->type.short_str()));
}

void dnd::ListVisitor::visit(const Feature* feature_ptr) {
    std::filesystem::path feature_path = std::filesystem::relative(
        feature_ptr->get_source_path(), feature_ptr->get_source_path().parent_path().parent_path()
    );
...
void dnd::ListVisitor::visit(const Choosable* choosable_ptr) {
    std::filesystem::path choosable_path = std::filesystem::relative(
        choosable_ptr->get_source_path(), choosable_ptr->get_source_path().parent_path().parent_path()
    );
