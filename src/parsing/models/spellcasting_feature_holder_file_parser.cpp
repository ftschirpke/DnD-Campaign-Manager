...
void dnd::SpellcastingFeatureHolderFileParser::parseSpellcasting() {
    ...
    if (!isAbility(ability)) {
        throw invalid_attribute(filepath, "spellcasting:attribute", "is not an ability");
    }
    ...
    if (has_spells_known && has_preparation_caster) {
        throw invalid_attribute(
            filepath, "spellcasting:spells_known/preparation_caster", "attributes are mutally exclusive."
        );
        ...
        if (preparation_spellcasting_type_str == "half") {
            ...
            throw invalid_attribute(filepath, "spellcasting:preparation_caster", "must be \"half\" or \"full\".");
        }
    } else {
        throw attribute_missing(filepath, "spellcasting:spells_known or spellcasting:preparation_caster");
    }
